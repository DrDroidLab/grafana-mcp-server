...
class GrafanaMCPClient:
    ...
    def list_tools(self) -> List[Dict[str, Any]]:
        ...
        try:
            response = self.test_client.post(
                "/mcp", data=json.dumps({"jsonrpc": "2.0", "method": "tools/list", "params": {}, "id": "tools-list"}), content_type="application/json"
            )
    ...
    def execute_tool(self, tool_name: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            response = self.test_client.post(
                "/mcp",
                data=json.dumps(
                    {"jsonrpc": "2.0", "method": "tools/call", "params": {"name": tool_name, "arguments": parameters}, "id": f"tool-{tool_name}"}
                ),
                content_type="application/json",
            )
    ...
    def promql_query(self, query: str, start_time: str, end_time: str, step: str = "60s", datasource_uid: Optional[str] = None) -> Dict[str, Any]:
        """Execute PromQL query via MCP server."""
        params = {"query": query, "start_time": start_time, "end_time": end_time, "step": step}
        if datasource_uid:
            params["datasource_uid"] = datasource_uid
        return self.execute_tool("grafana_promql_query", params)

    def loki_query(self, query: str, start_time: str, end_time: str, limit: int = 100, datasource_uid: Optional[str] = None) -> Dict[str, Any]:
        """Execute Loki query via MCP server."""
        params = {"query": query, "start_time": start_time, "end_time": end_time, "limit": limit}
        if datasource_uid:
            params["datasource_uid"] = datasource_uid
        return self.execute_tool("grafana_loki_query", params)

    def query_dashboard_panels(
        self, dashboard_uid: str, start_time: str, end_time: str, variables: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """Query dashboard panels via MCP server."""
        params = {"dashboard_uid": dashboard_uid, "start_time": start_time, "end_time": end_time}
        if variables:
